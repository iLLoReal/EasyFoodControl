...
import DisplayMeal from '../Display/DisplayMeal';
...
//  const { auth } = state;
  ...
  const handleLogout = async () => {
    await LoginApiCalls.logout(state.auth);
    dispatch({type: actions.LOGOUT, payload: [...state.recipes]});
    history.push("/");
  };
    ...
    <div>
      ...
      >
        <ul ref={nodeRef}>
            ...
            <li><NavLink to={'/Logout'} activeClassName={"white-nav-title"} onClick={handleLogout}><span className={isCurrent("/login") ? "current-white" : ""}>Logout</span></NavLink></li>
        </ul>
    </CSSTransition>
    </nav>
    <Switch>
      <Route path='/addmeal' exact component={DisplayMeal}/>
